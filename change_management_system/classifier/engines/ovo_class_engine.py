...
def tryIt(string, randFunction):
        ...
        if not os.path.exists('models/ovoclass/'+f_name):
                os.makedirs('models/ovoclass/'+f_name)
        else:
                exists = exists + 1
                
        if not os.path.exists('models/ovoclass/dump/'+f_name):
                os.makedirs('models/ovoclass/dump/'+f_name)
        else:
                exists = exists + 1

        if exists < 1 :      
                joblib.dump(clf, 'models/ovoclass/dump/'+f_name+'/OVO_SVM_' + string + '.joblib')
                i = 0

                for estimator in clf.estimators_:
                        writeSupportVectors('models/ovoclass/' + f_name + '/' + str(i) + '_OVO_SVM_' + string + f_name, estimator)
                        i = i + 1
