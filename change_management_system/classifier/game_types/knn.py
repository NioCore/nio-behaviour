...
def retryIt(string, randFunction):
        ...
        f = randFunction[1]
        multiSVM = loadOCSVMbySupportVectors('knnGame' + string + f.__name__)
...
def tryIt(string, randFunction):
        ...
        if not os.path.exists('models/game_types/knn/'+f_name):
                os.makedirs('models/game_types/knn/'+f_name)
        else:
                exists = exists + 1

        if not os.path.exists('models/game_types/knn/dump/'+f_name):
                os.makedirs('models/game_types/knn/dump/'+f_name)
        else:
                exists = exists + 1

        if exists < 1 : 
                joblib.dump(clf, 'models/game_types/knn/dump/'+f_name+'/KNN_' + string + '.joblib')
                # writeToFile('models/game_types/knn/'+f_name+'/KNN_' + string + f_name, clf)
