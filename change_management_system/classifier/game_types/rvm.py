...
def calcComplexScore(f_name, string, testClasses, checkersResult, chessResult, chineseResult):
        ...
        f = open("GameType_RVM_Results_"  + f_name + "_" + string + "_" + str(datetime.now().strftime('%Y-%m-%d_%H-%M-%S')) + ".result","w+")
...
def tryIt(string, randFunction):
        ...
        checkersSVM = EMRVC(kernel='rbf')
        chessSVM = EMRVC(kernel='rbf')
        chineseSVM = EMRVC(kernel='rbf')
        ...
        if not os.path.exists('models/game_types/rvm_oneclass/'+f_name):
                os.makedirs('models/game_types/rvm_oneclass/'+f_name)
        else:
                exists = exists + 1
        
        if not os.path.exists('models/game_types/rvm_oneclass/dump/'+f_name):
                os.makedirs('models/game_types/rvm_oneclass/dump/'+f_name)
        else:
                exists = exists + 1

        if exists < 1 :
                joblib.dump(checkersSVM, 'models/game_types/rvm_oneclass/dump/'+f_name+'/checkersRVM_' + string + '.joblib')
                joblib.dump(chessSVM, 'models/game_types/rvm_oneclass/dump/'+f_name+'/chessRVM_' + string + '.joblib')
                joblib.dump(chineseSVM, 'models/game_types/rvm_oneclass/dump/'+f_name+'/chineseRVM_' + string + '.joblib')

                writeSupportVectors('models/game_types/rvm_oneclass/' + f_name + '/checkersRVM_' + string + f_name, checkersSVM)
                writeSupportVectors('models/game_types/rvm_oneclass/' + f_name + '/chessRVM_' + string + f_name, chessSVM)
                writeSupportVectors('models/game_types/rvm_oneclass/' + f_name + '/chineseRVM_' + string + f_name, chineseSVM)
