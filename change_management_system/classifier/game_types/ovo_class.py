...
def retryIt(string, randFunction): 
        ...
        f = randFunction[1]
        multiSVM = loadOCSVMbySupportVectors('ovoGame' + string + f.__name__)
...
def tryIt(string, randFunction):
        ...
        clf = OneVsOneClassifier(_svc)
        clf.fit(X, y)

        exists = 0

        if not os.path.exists('models/game_types/ovoclass/'+f_name):
                os.makedirs('models/game_types/ovoclass/'+f_name)
        else:
                exists = exists + 1

        if not os.path.exists('models/game_types/ovoclass/dump/'+f_name):
                os.makedirs('models/game_types/ovoclass/dump/'+f_name)
        else:
                exists = exists + 1
        
        if exists < 1 :      
                joblib.dump(clf, 'models/game_types/ovoclass/dump/'+f_name+'/OVO_SVM_' + string + '.joblib')
                i = 0

                for estimator in clf.estimators_:
                        writeSupportVectors('models/game_types/ovoclass/' + f_name + '/' + str(i) + '_OVO_SVM_' + string + f_name, estimator)
                        i = i + 1
